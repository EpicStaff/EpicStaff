...
def cmd_create_crew(args):
    ...
    cid = result.get("id")
    print(
        f"Created crew: [{cid}] {result.get('name')} (process={result.get('process')})"
    )
    print("  ⚠️  REMINDER: When adding this crew to a flow, the crew node needs:")
    print(
        '     - input_map: map node parameters to flow variables (e.g. {"topic": "variables.request.topic"})'
    )
    print(
        "     - output_variable_path: set to 'variables' or 'variables.<domain>' to store crew output"
    )
    return result
...
def cmd_create_agent(args):
    ...
    if not llm_config_id:
        print(
            "  ⚠️  WARNING: No LLM config set. Agent will not work until an LLM config is assigned."
        )
