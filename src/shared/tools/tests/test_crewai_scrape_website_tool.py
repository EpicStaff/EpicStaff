...
scrape_module = load_tool_main("crewai_scrape_website_tool")
scrape_main = scrape_module.main

HTML_BODY = "<html><body><p>Hello</p></body></html>"


class TestScrapeWebsiteToolSsrf:
    def test_ssrf_refuses_loopback_address(self):
        result = scrape_main(website_url="http://127.0.0.1/")

        assert result.startswith("Error:")
        assert "SSRF" in result

    def test_ssrf_refuses_link_local_metadata_address(self):
        result = scrape_main(website_url="http://169.254.169.254/")

        assert result.startswith("Error:")
        assert "SSRF" in result

    def test_ssrf_refuses_invalid_scheme(self):
        result = scrape_main(website_url="ftp://example.com/file")

        assert result.startswith("Error:")
        assert "scheme" in result


class TestScrapeWebsiteToolRedirects:
    def test_redirect_to_loopback_is_refused(self, monkeypatch):
        ...
        monkeypatch.setattr(
            scrape_module.requests, "get", make_fake_requests_get(responses)
        )

        result = scrape_main(website_url=url)
    ...
    def test_redirect_loop_hits_hop_cap(self, monkeypatch):
        ...
        assert str(scrape_module.MAX_REDIRECTS) in result
