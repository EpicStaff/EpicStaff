...
from infrastructure.file_text_extractors.strategies.html_extractor import (
    HTMLTextExtractor,
)
from tests.conftest import offload_to_process

SAMPLE_HTML = b'<html><body><p>Hi</p><script>bad()</script><img src="x"><style>.c{}</style></body></html>'


async def test_strips_script_style_img():
    assert (
        await HTMLTextExtractor().extract(SAMPLE_HTML)
...
async def test_extract_runs_in_process():
    ...
    result, worker_pid = await offload_to_process(
        lambda: extractor.extract(SAMPLE_HTML)
    )
