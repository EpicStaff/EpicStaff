...
async def test_get_config_called_with_correct_rag_id(repo, uow, monkeypatch):
    ...
    request = SearchRequest(
        rag_id=77, query="q", search_config=GraphGlobalSearchConfig()
    )
    await GraphSearch(uow).on_execute(request)
...
async def test_resolve_files_called_with_correct_parameters(uow, monkeypatch):
    ...
    monkeypatch.setattr(
        GraphSearch, "_resolve_files", staticmethod(recording_resolve_files)
    )
    ...
    expected_required = set(
        GraphSearch._SEARCH_MAP[GraphSearchMethodEnum.LOCAL].required_files
    )
