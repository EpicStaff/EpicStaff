...
- **Use combinations wisely**: Some PRs legitimately need multiple labels
  - Example: New feature with security implications → ["feature", "security"]
  - Example: Bug fix that improves performance → ["bug", "performance"]
...
### Highlight Impact
- For breaking changes: "⚠️ Requires migration"
- For major features: "🎉 Highly requested feature"
- For security: "🔒 Security fix"
...
### Good Feature Note:
"✨ **Analytics Dashboard**: View detailed usage metrics, performance trends, and user activity. Access via Settings → Analytics."

### Good Breaking Change Note:
"⚠️ **Authentication API Changes**: The `/auth/login` endpoint now requires a `client_id` parameter. See migration guide for details."

### Good Bug Fix:
"🐛 Fixed crash when uploading files larger than 100MB (#245)"
