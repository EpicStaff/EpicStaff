...
class FlowCodeUsageSource(UsageSource):
    """A graph node whose PythonCode declares secrets.

    Reads the declaration rather than scanning the code: once the allow-list is the
    contract, "references this secret" means "is allowed to read it", which is
    exactly what deleting the secret would take away. A node that merely names a
    secret it did not declare cannot be saved and is rejected at session start, so
    it is not a user of it.
    """
    ...
    def collect(self, *, org_id: int, secret_names: dict[str, int]) -> list[UsageHit]:
        rows = self.model.objects.filter(
            graph__org_id=org_id,
            **{f"{self.code_field}__secrets__id__in": set(secret_names.values())},
        ).values_list(
            f"{self.code_field}__secrets__id",
            "graph_id",
            "graph__name",
            "node_name",
        )
...
class ToolCodeUsageSource(UsageSource):
    ...
    A built-in tool declaring "X" resolves the *querying* org's X at run time, so
    deleting X really would break that tool for this org. org_visible_q is the same
    rule the RBAC guide mandates for hybrid targets.
    """

    def collect(self, *, org_id: int, secret_names: dict[str, int]) -> list[UsageHit]:
        rows = (
            org_visible_queryset(model=PythonCodeTool, org_id=org_id)
            .filter(python_code__secrets__id__in=set(secret_names.values()))
            .values_list("python_code__secrets__id", "name")
        )
    ...
    def collect(self, *, org_id: int, secret_names: dict[str, int]) -> list[UsageHit]:
        rows = list(
            ConditionalEdge.objects.filter(
                graph__org_id=org_id,
                python_code__secrets__id__in=set(secret_names.values()),
            ).values_list(
                "python_code__secrets__id",
                "graph_id",
                "graph__name",
                "source_node_id",
                "id",
