...
def _clause_set(expression: str) -> set[str]:
    return {" ".join(part.split()) for part in (expression or "").split(" and ") if part.strip()}
...
def _render_conditions(block: dict) -> list[str]:
    ...
    if field_clauses:
        redundant = expression and _clause_set(" and ".join(field_clauses)) <= _clause_set(
            expression
        )
        if not redundant:
...
def _render_condition_fields(block: dict) -> list[str]:
    ...
    lines.append(
        f"  on match, runs the AI prompt: {prompt}" if prompt else "  on match, runs no AI prompt"
    )
    lines.append(
    ...
    if route_code:
        lines.append(
            f"  outgoing connector label: {route_code} (a label only — it does not affect routing)"
        )
    return lines
...
def _render_prompt_fields(block: dict) -> list[str]:
    ...
    lines += _render_mapping(
        "after the answer returns, these values are filled from its fields", mappings
    )
