...
- `inline_surface` — an `InlineSurface` on a `TaskNode` (via
...
class _ToolKindConfig:
    """One registry entry per tool kind, bundling everything the dispatch
    functions below need. A third tool kind is a single new entry here
    instead of coordinated edits across separate model/field dicts and an
    `if/elif` chain."""

    catalog_model: type
    task_inline_model: type
    agent_inline_model: type
    tool_field: str
    usage_fn: Callable[[int, dict], list[dict]]
...
def get_tools_usage(
    ...
    id_q = {"id__in": ids} if ids else {}
    return config.usage_fn(org_id, id_q)
...
_TOOL_KIND_REGISTRY: dict[type, _ToolKindConfig] = {
    PythonCodeTool: _ToolKindConfig(
        catalog_model=SurfacePythonTool,
        task_inline_model=InlineSurfacePythonTool,
        agent_inline_model=AgentInlineSurfacePythonTool,
        tool_field="python_tool_id",
        usage_fn=_get_python_code_tool_usage,
    ),
    McpTool: _ToolKindConfig(
        catalog_model=SurfaceMcpTool,
        task_inline_model=InlineSurfaceMcpTool,
        agent_inline_model=AgentInlineSurfaceMcpTool,
        tool_field="mcp_tool_id",
        usage_fn=_get_mcp_tool_usage,
    ),
}
...
def _surfaces_by_tool(
    ...
    "inline_surface": [...]}` buckets across the three attachment families. Each
    ...
    the same tool inline produce two separate `inline_surface` entries). Still ONE
    ...
    `owner_agent_id`; the two inline families both feed the single `inline_surface`
    bucket."""
    ...
    for tool_id, entry in _task_inline_surface_entries(org_id, tool_ids, tool_class):
        surfaces_by_tool[tool_id]["inline_surface"].append(entry)
...
def _catalog_surface_entries(org_id: int, tool_ids: list[int], tool_class: type):
    ...
    buckets in Python."""
    config = _TOOL_KIND_REGISTRY[tool_class]
    ...
    for tool_id, surface_id, surface_name, owner_agent_id in rows:
        ...
        yield tool_id, bucket, {"id": surface_id, "name": surface_name, "node_id": None}
...
def _task_inline_surface_entries(org_id: int, tool_ids: list[int], tool_class: type):
    ...
    config = _TOOL_KIND_REGISTRY[tool_class]
    inline_model = config.task_inline_model
    tool_field = config.tool_field
...
def _agent_inline_surface_entries(org_id: int, tool_ids: list[int], tool_class: type):
    ...
    config = _TOOL_KIND_REGISTRY[tool_class]
    agent_inline_model = config.agent_inline_model
    tool_field = config.tool_field
    ...
    rows = agent_inline_model.objects.filter(
        ...
        **{f"{tool_field}__in": tool_ids},
        ...
        "agent_inline_surface__agent_node_id",
    )
...
def get_python_code_tool_usage_detail(tool_id: int, org_id: int) -> dict:
    ...
    "inline_surface": [...]}`. Raises `ToolNotFoundError` if the tool doesn't exist /
        ...
        rows.append(
            {
                ...
                "inline_surface_count": len(buckets["inline_surface"]),
