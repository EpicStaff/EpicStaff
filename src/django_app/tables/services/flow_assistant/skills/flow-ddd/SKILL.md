...
| Any work that needs LLM reasoning (decide, summarize, compose, converse) | `project` (crew node) |
...
- **If the logic needs reasoning over fuzzy text, use `project` (crew).** It is the only node type that runs LLM reasoning.
