...
class TestMoveFlows:
    def test_move_file_into_folder_creates_row_at_nested_path(
        ...
        manager.mkdir("testuser", org.id, "archive")
        manager.upload("testuser", org.id, "report.txt", BytesIO(b"data"))

        manager.move("testuser", org.id, "report.txt", "archive")
    ...
    def test_move_file_into_folder_with_name_collision_dedupes(
        ...
        manager.mkdir("testuser", org.id, "archive")
        manager.upload("testuser", org.id, "archive/report.txt", BytesIO(b"existing"))
        manager.upload("testuser", org.id, "report.txt", BytesIO(b"incoming"))
    ...
    def test_move_folder_into_folder_creates_nested_rows_and_keeps_target(
        ...
        manager.mkdir("testuser", org.id, "archive")
        manager.mkdir("testuser", org.id, "docs")
        manager.upload("testuser", org.id, "docs/a.txt", BytesIO(b"a"))

        manager.move("testuser", org.id, "docs", "archive")
    ...
    def test_move_folder_into_folder_with_name_collision_dedupes(
        ...
        manager.mkdir("testuser", org.id, "archive/docs")
        manager.mkdir("testuser", org.id, "docs")
        manager.upload("testuser", org.id, "docs/a.txt", BytesIO(b"a"))
...
class TestCopyFlow:
    def test_copy_folder_creates_db_rows_visible_via_list_and_tree(
        ...
        manager.copy("testuser", org.id, "docs", "")
        ...
        root, _ = manager.list_tree("testuser", org.id, "docs (1)")
...
class TestRenameFlow:
    def test_rename_onto_existing_path_raises_and_leaves_db_unchanged(
        ...
        manager.upload("testuser", org.id, "a.txt", BytesIO(b"a"))
        manager.upload("testuser", org.id, "b.txt", BytesIO(b"b"))

        with pytest.raises(FileExistsError):
            manager.rename("testuser", org.id, "a.txt", "b.txt")
...
class TestDownloadFlow:
    def test_download_without_db_row_raises_file_not_found(
        ...
        with pytest.raises(FileNotFoundError):
            manager.download("testuser", org.id, "stray.txt")

    def test_download_with_db_row_returns_bytes(self, manager, org, org_user):
        manager.upload("testuser", org.id, "known.txt", BytesIO(b"known content"))

        assert manager.download("testuser", org.id, "known.txt") == b"known content"
...
class TestCrossOrgMoveFlow:
    def test_move_cross_org_creates_dest_row_and_removes_source_row(
        ...
        manager.upload("testuser", org.id, "shared.txt", BytesIO(b"payload"))
        manager.mkdir("testuser", second_org.id, "inbox")

        manager.move_cross_org("testuser", org.id, "shared.txt", second_org.id, "inbox")
