...
class Memory(MemoryBase):
    ...
    def add(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future1 = executor.submit(
                self._add_to_vector_store, messages, metadata, filters, search_limit
            )
            future2 = (
                executor.submit(self._add_to_graph, messages, filters)
                if self.api_version == "v1.1"
                else None
            )

            concurrent.futures.wait([future1, future2] if future2 else [future1])

            vector_store_result = future1.result()
            graph_result = future2.result() if future2 else None
    ...
    def _add_to_vector_store(self, messages, metadata, filters, search_limit):
