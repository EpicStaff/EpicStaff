...
class CrewAgentExecutor(CrewAgentExecutorMixin):
    ...
    def __init__(
        ...
        user_input_contextual_memory: Any = None,
    ):
        ...
        self.user_input_contextual_memory = user_input_contextual_memory
    ...
    def _handle_regular_feedback(
        ...
        while self.ask_for_human_input:
            user_feedback = feedback.split(KNOWLEDGE_KEYWORD, 1)[0].strip()

            if user_feedback == "</done/>" or "</done/>" in user_feedback:
                self.ask_for_human_input = False
                logger.info("End of user input")
            else:
                try:
                    if (
                        self.crew
                        and self.crew.memory
                        and self.crew._user_memory is not None
                    ):
                        user_memories = (
                            self.user_input_contextual_memory._fetch_user_context(
                                query=user_feedback, limit=8, score_threshold=0.7
                            )
                        )
                        feedback += user_memories
                        self._create_user_memory(user_feedback)
                        logger.info("Searching and adding user memories completed.")

                except Exception as e:
                    logger.error(f"Error fetching user memories during user input: {e}")
