...
export class ApiKeysTabComponent {
    ...
    protected totalApiKeysCount: number | null = null;
    ...
    constructor() {
        ...
        this.filters$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadApiKeys());
    }

    get canFilterByOrg(): boolean {
    ...
    onBulkRevoke(): void {
        ...
        if (!count) {
            if (isSomeRevoked) this.toast.info('Selected keys is already revoked');
            return;
        }
        ...
        this.confirmation
            ...
            .subscribe({
                next: () => {
                    if (isSomeRevoked) this.toast.info('Some of selected keys was revoked before');
    ...
    private loadApiKeys(): void {
        ...
        // For superadmins the org override is always set (null = all orgs, number = specific org).
        // For non-superadmins it stays undefined so the interceptor uses the active org as normal.
        ...
        this.apiKeysService
            ...
            .subscribe({
                next: (keys) => {
                    this.totalApiKeysCount ??= keys.length;
                    this.keys.set(keys);
                    this.rememberOwners(keys);
                },
                error: (err) => this.toast.error(err.error?.message ?? 'Failed to reload API keys'),
            });
    }
