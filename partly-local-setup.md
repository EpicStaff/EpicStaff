...
Load env vars (requires `src/.debug.env`, generated by `scripts/generate_env.py`; it's gitignored, so generate it first if missing), set `PYTHONPATH` so `from src.shared...` imports resolve, then migrate and start the server:
```powershell
Get-Content ..\.debug.env | Where-Object { $_ -and $_ -notmatch '^#' } | ForEach-Object {
