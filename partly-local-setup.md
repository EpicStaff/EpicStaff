...
Load env vars (requires `src/.env`, generated by `scripts/envtool.py`; it's gitignored, so generate it first if missing — `python scripts/envtool.py --dev` from the repo root), set `PYTHONPATH` so `from src.shared...` imports resolve, then migrate and start the server:
```powershell
