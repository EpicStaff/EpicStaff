...
Local dev (`src/.env` generated by `python scripts/envtool.py --dev`) sets
`DJANGO_FIRST_SETUP_MODE=open`, so local development keeps the browser setup
flow: visiting the frontend with no superadmin yet shows the setup form, and
submitting it creates the first superadmin over HTTP.
