...
| `litellm` model-price JSON fetch (`model_prices_and_context_window.json`, historically fetched from a GitHub raw URL on some versions/paths) | `import litellm` / first cost-calculation call, used transitively by crewAI's `LLM` wrapper (`PatchedLLM`), `django_app`'s `litellm_client.py`, and GraphRAG's litellm-based language model provider | **Unconfirmed** — no `LITELLM_LOCAL_MODEL_COST_MAP` override found in any env file or first-party code; litellm 1.75.7 ships a local backup JSON but its exact fetch-vs-local-first behavior needs a runtime network trace to confirm for this pinned version | Would be an outbound GET to a litellm-controlled GitHub URL; no EpicStaff/customer data leaves | **Needs verification** | Capture container network traffic on `crew`/`knowledge`/`django_app` startup and first LLM call; if a fetch occurs, set `LITELLM_LOCAL_MODEL_COST_MAP=True` in the shared env |
...
### 7. litellm model-cost JSON — unconfirmed, needs a network trace

- `litellm==1.75.7` is a direct dependency of `src/crew/pyproject.toml:33` and is used inside crewAI's `LLM` class (`src/crew/libraries/crewAI/src/crewai/llm.py`) which `PatchedLLM` (`src/crew/utils/llm_wrapper.py`) wraps; also used by `src/django_app/tables/services/llm_clients/litellm_client.py` and GraphRAG's `graphrag/language_model/providers/litellm/*`.
- No `LITELLM_LOCAL_MODEL_COST_MAP` override was found anywhere in `src/` env files or first-party code. litellm's behavior around fetching its live cost-map JSON from GitHub vs. using the bundled backup varies by version and call path; I could not confirm from static reading alone whether the pinned `1.75.7` performs a network fetch on `import litellm` or only when specific cost/usage APIs are called (which EpicStaff may or may not invoke). This needs a runtime capture (e.g. `tcpdump`/`mitmproxy` on the `crew` container during startup and a first LLM call) rather than a code-reading guess.
...
1. **Fix (must-do before EULA ships):** Pre-cache the `gpt2` HuggingFace tokenizer file used by chonkie's default `TokenChunker` at `knowledge` image build time, and set `HF_HUB_OFFLINE=1` at runtime so any future accidental HF Hub call fails loudly instead of phoning home. (`src/knowledge/chunkers/token_chunker.py`, `src/knowledge/Dockerfile.knowledge`)
2. **Fix (must-do before EULA ships):** Pre-cache tiktoken encodings (`cl100k_base` and whatever GraphRAG's `ENCODING_MODEL` default resolves to) at build time for both the `knowledge` and `realtime` images, and set `TIKTOKEN_CACHE_DIR` to a baked-in path. (`src/knowledge/libraries/graphrag/graphrag/tokenizer/get_tokenizer.py`, `src/realtime/utils/tokenizer.py`, `src/knowledge/Dockerfile.knowledge`, `src/realtime/Dockerfile.realtime`)
3. **Cleanup:** Remove (or fix) the `huggingface` embedder catalog entry in `src/django_app/tables/provider_models/embedding_models.json` — it currently crashes with `ImportError` if selected, and if `sentence-transformers` is ever added to fix that, it must ship with a build-time model cache or it becomes a real silent-download regression.
4. **Verify:** Confirm `MEM0_TELEMETRY=False` (or equivalent) is actually set in every deployment target (K8s manifests, customer install scripts), not just the reference `docker-compose.yaml`. Upstream mem0 defaults to telemetry-on if the env var is absent.
5. **Verify (needs a runtime trace, not just code reading):** Determine whether `litellm==1.75.7` fetches its model-price JSON from GitHub at import or first use in this deployment; if so, set `LITELLM_LOCAL_MODEL_COST_MAP=True` across `crew`, `knowledge`, and `django_app`.
