...
- Gotcha found during this audit: the code reads `MONITORING_TYPE` (`telemetry.py:40`) but the shipped env config used to define `MONITORING_MODE` instead — a name mismatch that meant `MONITORING_TYPE` was never actually set, so the code always fell through to `MonitoringType.LOCAL` regardless of what operators put in `MONITORING_MODE`. As of the env/DI migration the variable was dropped entirely: neither `src/env.yaml`, `src/.env.example`, nor `src/docker-compose.yaml` declares any `MONITORING_*` variable anymore, so telemetry is now hardwired to the fail-safe local-only path. If a server-monitoring option is ever reintroduced, declare it in `src/env.yaml` as `MONITORING_TYPE` (matching the code) so `MONITORING_TYPE=server` actually takes effect.
